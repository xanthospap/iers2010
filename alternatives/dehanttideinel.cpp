 ...
 * @param[in]  timesys [optional] if the time argument(s) is given in TAI 
 *                     instead of TT, set this to 'TAI'. If this string is not
 *                     'TAI' it is never used.
 ...
 *       expressed in meters.
 *    -# Parameters mjd1 and mjd2 constitute the date as Modified Julian Date in 
 *       TT time scale. The actual date is given by the addition mjd1+mjd2. 
 *       Either mjd1 or mjd2 can be set to zero. Note that the time scale can
 *       also be TAI, but at that case, you should se the 'timesys' string to
 *       'TAI'.
 *    -# Status: Class 1
 ...
 * 
 * WARNING This produces small discrepancies; need to work on time transformations
 * 
 */
int iers2010::dehanttideinel (const double* xsta,const double* xsun,
    const double* xmon,const double& mjd1,const double& mjd2,
    double* dxtide,const char* timesys)
{

    double xcorsta[] = {.0e0,.0e0,.0e0};

    //  fractional day difference between TAI and TT (i.e TT = TAI + 32.184sec)
    constexpr double TT2TAI ( 32.184e0/(3600.0e0*24.0e0) );
 ...
 *----------------------------------------------------------------------*/
    ...
    //  Convert Modified Julian Date to Julian Date,  i.e.
    //  MJD = JD - 2400000.5
    //  and then to Julian Centuries
    //  WARNING: If the time argument is given in TAI, then we should add
    //           seconds to convert it to TT (32.184s).
    double mjt1 (mjd1),  mjt2 (mjd2);
    if (mjd1<mjd2) {
        mjt1 = mjd2;
        mjt2 = mjd1;
      }
    ...
    double t ( mjt1/36525e0 + mjt2/36525e0 );

    // Compute the fractional part of day (this is just the fractional
    // part of the MJD)
    double intpart;
    double fhr ( std::modf (mjd1+mjd2,&intpart) );
    //  If time argument given in TAI,  convert it to TT
    if (timesys)
    if ( !strcmp (ctai,timesys) )
    fhr += TT2TAI;
    printf ("\n\t\t%20.18f %20.18f", t, fhr);
