 ...
 * @return T = Q * R * W
 */
Eigen::Matrix<double, 3, 3> T(const Eigen::Matrix<double, 3, 3> &Q, double era,
...
class Itrs2Gcrs {
  ...
  /* Precession/Nutation matrix: transforms GCRS-to-CIRS matrix (i.e. Q^t
   * in 5.1) */
...
public:
  ...
  /* @brief Get current TT (MJD) */
  dso::TwoPartDate tt() const noexcept { return t_tt; }
  /* @brief Get current TAI (MJD) */
  dso::TwoPartDate tai() const noexcept { return t_tt.tt2tai(); }
  /* @brief Get current UT1 (MJD) */
  dso::TwoPartDate ut1() const noexcept { return t_tt.tt2ut1(eops.dut); }
