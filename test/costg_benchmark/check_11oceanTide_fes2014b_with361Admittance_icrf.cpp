...
if (argc != 9) {
  fprintf(stderr,
          "Usage: %s [00orbit_itrf.txt] [11oceanTide_fes2014b_with361Admittance_icrf.txt] "
          "[01earthRotation_rotaryMatrix.txt] [eopc04.1962-now] "
          "[FES2014b_OCN_001fileList.txt] [FES2014b_OCN_002doodson.txt] [FES2014b_OCN_003admittance.txt] [FES2014b gfc dir]\n",
          argv[0]);
  return 1;
}
...
for (const auto &in : orbvec) {
  ...
  /* compute fundamental (Delaunay) arguments for t */
