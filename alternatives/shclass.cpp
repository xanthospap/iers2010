...
iers2010::SphericalHarmonics::SphericalHarmonics (const double& xi,
        const double& yi,const double& zi)
:  mdegree(9), morder(9), mx(xi), my(yi), mz(zi)
  {
    mv = new double* [mdegree+1];
    mw = new double* [mdegree+1];
    for (int i=0;i<=mdegree;i++) {
...
iers2010::SphericalHarmonics& iers2010::SphericalHarmonics::operator = 
(const SphericalHarmonics& s)
{
    if (this != &s) {
        mx = s.mx;
        my = s.my;
