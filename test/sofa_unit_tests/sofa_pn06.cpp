...
int main() {
  ...
  for (int i = 0; i < NUM_TESTS; i++) {
    ...
    pn06(tt, dpsi, deps, epsam, rbm, rpm, rbpm, rnm, rbpnm);
    iauPn06(tt.big() + dso::mjd0_jd, tt.small(), dpsi, deps, &epsas, rb, rp,
            rbp, rn, rbpn);
    /* rb */
    if (!approx_equal(rbm, rb)) {
      const double theta = rotation_matrix_diff(rbm, rb);
      ++fails[0];
      if (theta > std::abs(max_error[0])) {
        max_error[0] = theta;
      }
    }
    /* rp */
