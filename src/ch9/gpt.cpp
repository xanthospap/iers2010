...
int iers2010::gpt(double dmjd, double dlat, double dlon, double dhgt,
  ...
  const double sinlat = std::sin(dlat);
  const double coslon = std::cos(dlon);
  const double sinlon = std::sin(dlon);
  const double x = coslat * coslon;
  const double y = coslat * sinlon;
  ...
  for (int n = 1; n < nmax; n++) {
    N = n + 1;
    v[n + 1][0] =
        ((2 * N - 1) * z * v[n][0] - (N - 1) * v[n - 1][0]) / (double)N;
    w[n + 1][0] = 0e0;
  }
