...
int iers2010::oeop::cnmtx(double dmjd, double *h) noexcept {
  ...
  for (int k = -1; k < 2; k++) {
    ...
    anm2[mm-1][k + 1] = 0e0;
    bnm2[mm-1][k + 1] = 0e0;
    constexpr const double pinm = ((double)((nn + mm) % 2)) * TWOPI / 4e0;
    for (int j = 0; j < nlines21; j++) {
      alpha = std::fmod(x21[j].phase - pinm, TWOPI) +
              std::fmod(x21[j].freq * dt60, TWOPI);
      //printf("anm[2,%d,%d] += %15.6f * COS(%15.9f)\n", mm,k+1,x21[j].hs,alpha);
      anm2[mm-1][k + 1] += x21[j].hs * std::cos(alpha);
      bnm2[mm-1][k + 1] -= x21[j].hs * std::sin(alpha);
    }
    //printf("A(2,%d,%d)=%15.9f\n", mm, k, anm2[mm-1][k+1]);
  }
  ...
  for (int k = -1; k < 2; k++) {
    ...
    for (int j = 0; j < nlines22; j++) {
      //printf("anm[2,%d,%d] += %15.6f * COS(%15.9f)\n", mm,k+1,x22[j].hs,alpha);
      alpha = std::fmod(x22[j].phase - pinm, TWOPI) +
              std::fmod(x22[j].freq * dt60, TWOPI);
  ...
  // first loop
  //printf("A(2,1,-1)=%15.9f\n", anm2[0][0]);
  //printf("A(2,1, 0)=%15.9f\n", anm2[0][1]);
  //printf("A(2,1, 1)=%15.9f\n", anm2[0][2]);
  //printf("B(2,1,-1)=%15.9f\n", bnm2[0][0]);
  //printf("B(2,1, 0)=%15.9f\n", bnm2[0][1]);
  //printf("B(2,1, 1)=%15.9f\n", bnm2[0][2]);
  //// second loop
  //printf("A(2,2,-1)=%15.9f\n", anm2[1][0]);
  //printf("A(2,2, 0)=%15.9f\n", anm2[1][1]);
  //printf("A(2,2, 1)=%15.9f\n", anm2[1][2]);
  //printf("B(2,2,-1)=%15.9f\n", bnm2[1][0]);
  //printf("B(2,2, 0)=%15.9f\n", bnm2[1][1]);
  //printf("B(2,2, 1)=%15.9f\n", bnm2[1][2]);
