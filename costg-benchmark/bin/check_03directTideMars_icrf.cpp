...
constexpr const double GM_Mars    = 1.32712442076e20/3098708.0;
...
int main(int argc, char *argv[]) {
  ...
  if (formatD3Plot) {
    ...
    printf("#title Direct Tide Mars (ephermeris: %s)\n", basename(argv[3]));
  }
  ...
  for (const auto &in : orbvec) {
    ...
    /* get Mars position in ICRF */
    if (dso::planet_pos(dso::Planet::MARS, t, rtb)) {
      fprintf(stderr, "ERROR Failed to compute Mars position!\n");
      return 2;
    }
    /* compute third body acceleration for given epoch/position */
    a = dso::point_mass_acceleration(in.xyz, rtb, GM_Mars, g);
    /* get COSTG result */
