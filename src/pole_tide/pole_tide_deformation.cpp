...
dso::PoleTide::deformation(const dso::MjdEpoch &t, double xp, double yp,
  ...
  s(0) = -9e0 * c2t * (m1 * cl + m2 * sl);  /* Sθ along colatitude in [mm] */
  s(1) = 9e0 * ct * (m1 * sl - m2 * cl);    /* Sλ along longitude in [mm] */
  s(2) = -33e0 * s2t * (m1 * cl + m2 * sl); /* Sr radial in [mm] */
  ...
  return dso::CartesianCrd((R * s)*1e-3);
}
