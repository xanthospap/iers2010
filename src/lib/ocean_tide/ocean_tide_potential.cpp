...
int dso::OceanTide::stokes_coeffs_impl(
...
int dso::OceanTide::stokes_coeffs_with_admittance_impl(
    const dso::MjdEpoch &mjdtt, const dso::MjdEpoch &mjdut1,
    const double *const delaunay_args) noexcept {
  /* nullify geopotential coeffs */
  mcs.clear();
  ...
  // double __dargs[6];
  // dso::delaunay2doodson(delaunay_args, gmst, __dargs);
  // Eigen::Matrix<double, 6, 1> theta = Eigen::Map<Eigen::Matrix<double, 6,
  // 1>>(__dargs);
  ...
  // std::cout<<"admittance:\n"<<admittance_matrix()<<"\n";
  const Eigen::VectorXd factorCos =
      admittance_matrix() *
      thetaf.unaryExpr([](double x) { return std::cos(x); });
  const Eigen::VectorXd factorSin =
      admittance_matrix() *
      thetaf.unaryExpr([](double x) { return std::sin(x); });
  // std::cout << "factorCos=\n" << factorCos << "\n";
  ...
  for (const auto &wave : atlas().waves()) {
    mcs.Cnm() +=
        wave.stokes_cos().Cnm() * (*fcos) + wave.stokes_sin().Cnm() * (*fsin);
    mcs.Snm() +=
        wave.stokes_cos().Snm() * (*fcos) + wave.stokes_sin().Snm() * (*fsin);
