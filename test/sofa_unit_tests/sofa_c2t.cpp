...
dso::Mat3x3 c2t06a_v2(double tta, double ttb, double uta,
  ...
  // [GCRS] = Q R W [ITRS] (but we want the inverse transformation)
  // [ITRS] = W * R * Q [GCRS]
...
int main() {
  ...
  for (int t = 0; t < num_tests; t++) {
    ...
    // call version2 implementation
    dso::Mat3x3 c2t_v2 = c2t06a_v2(dso::mjd0_jd, mjd_tt, dso::mjd0_jd,
                                             mjd_ut1, xp, yp);
    // apply ...
    auto tvec_v2 = c2t_v2 * vec;
    ...
    for (int i = 0; i < 3; i++) {
      double d;
      if ((d = std::abs(tvec(i) - tvec_v2(i))) > diffs(i)) {
        diffs(i) = d;
      }
      avediffs(i) += tvec(i) - tvec_v2(i);
    }
