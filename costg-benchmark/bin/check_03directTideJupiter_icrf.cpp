...
constexpr const double GM_Jupiter    = 1.32712442076e20/1047.3486;
...
int main(int argc, char *argv[]) {
  ...
  if (formatD3Plot) {
    ...
    printf("#title Direct Tide Jupiter (ephermeris: %s)\n", basename(argv[3]));
  }
  ...
  for (const auto &in : orbvec) {
    ...
    /* get  Jupiter position in ICRF */
    if (dso::planet_pos(dso::Planet::JUPITER, t, rtb)) {
      fprintf(stderr, "ERROR Failed to compute Jupiter position!\n");
      return 2;
    }
    /* compute third body acceleration for given epoch/position */
    a = dso::point_mass_acceleration(in.xyz, rtb, GM_Jupiter, g);
    /* get COSTG result */
