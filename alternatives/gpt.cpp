		...
		const double& dhgt,double& pres,double& temp,double& undu)
{
    ...
    static constexpr double bt_mean [] = {   
  ...
  const double& dhgt,double& pres,double& temp,double& undu)
{
  ...
  static iers2010::SphericalHarmonics sh;
  if (fabs(x-sh.x())>1e-03 ||  fabs(y-sh.y())>1e-03 ||  fabs(z-sh.z())>1e-03 ) {
    sh.reset (x, y, z);
    //printf ("\n\tRECOMPUTED!");
  }
  ...
  for (int n=0;n<=nmax;n++) {
    for (int m=0;m<=n;m++) {
      undu += ( a_geoid[i] * sh.v(n,m) + b_geoid[i] * sh.w(n,m) );
      i++;
     }
   }
  ...
  for (int n=0;n<=nmax;n++) {
    for (int m=0;m<=n;m++) {
      apm += ( ap_mean[i] * sh.v(n,m) + bp_mean[i] * sh.w(n,m) );
      apa += ( ap_amp[i] * sh.v(n,m) + bp_amp[i] * sh.w(n,m) );
      i++;
     }
   }
  double pres0 ( apm + apa * cos (doy/365.25e0*TWOPI) );
  ...
  for (int n=0;n<=nmax;n++) {
    for (int m=0;m<=n;m++) {
      atm += ( at_mean[i] * sh.v(n,m) + bt_mean[i] * sh.w(n,m) );
      ata += ( at_amp[i] * sh.v(n,m) + bt_amp[i] * sh.w(n,m) );
      i++;
     }
   }
  double temp0 ( atm + ata * cos (doy/365.25e0*TWOPI) );
...
int iers2010::gpt3 (const double& dmjd,const double& dlat,const double& dlon,
                    const double& dhgt,double& pres,double& temp,double& undu)
{
    #ifdef USE_EXTERNAL_CONSTS
    constexpr double TWOPI   (D2PI);
