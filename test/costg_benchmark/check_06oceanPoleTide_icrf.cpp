...
int main(int argc, char *argv[]) {
  ...
  for (const auto &in : orbvec) {
    ...
    if (opt.stokes_coeffs(t, xp, yp, DEGREE, ORDER)) {
      fprintf(stderr, "ERROR Failed computing Stokes COefficients\n");
      return 1;
    }

    /* for the test, we only need C and S for n,m >= 2 */
    opt.stokes_coeffs().C(0,0) = opt.stokes_coeffs().C(1,0) = opt.stokes_coeffs().C(1,1) = 0e0;
    opt.stokes_coeffs().S(0,0) = opt.stokes_coeffs().S(1,0) = opt.stokes_coeffs().S(1,1) = 0e0;
