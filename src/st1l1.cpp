...
///       96, 8243-8257
void iers2010::dhtide::st1l1(const double *xsta, const double *xsun,
                             const double *xmon, double fac2sun, double fac2mon,
                             double *xcorsta) noexcept {
  const double l1d{0.0012e0}, l1sd{0.0024e0};

  // Compute the normalized position vector of the IGS station.
  const double rsta{sqrt(std::inner_product(xsta, xsta + 3, xsta, .0e0))};
  ...
  dnsun = -l1 / 2e0 * sinphi * cosphi * fac2sun *
          ((pow(xsun[0], 2) - pow(xsun[1], 2)) * costwola +
           2e0 * xsun[0] * xsun[1] * sintwola) /
          rsun2;

  dnmon = -l1 / 2e0 * sinphi * cosphi * fac2mon *
          ((pow(xmon[0], 2) - pow(xmon[1], 2)) * costwola +
           2e0 * xmon[0] * xmon[1] * sintwola) /
          rmon2;

  desun = -l1 / 2e0 * sinphi2 * cosphi * fac2sun *
          ((pow(xsun[0], 2) - pow(xsun[1], 2)) * sintwola -
           2e0 * xsun[0] * xsun[1] * costwola) /
          rsun2;

  demon = -l1 / 2e0 * sinphi2 * cosphi * fac2mon *
          ((pow(xmon[0], 2) - pow(xmon[1], 2)) * sintwola -
           2e0 * xmon[0] * xmon[1] * costwola) /
          rmon2;
