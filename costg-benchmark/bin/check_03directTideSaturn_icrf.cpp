...
constexpr const double GM_Saturn    = 1.32712442076e20/3497.898;
...
int main(int argc, char *argv[]) {
  ...
  if (formatD3Plot) {
    ...
    printf("#title Direct Tide Saturn (ephermeris: %s)\n", basename(argv[3]));
  }
  ...
  for (const auto &in : orbvec) {
    ...
    /* get Saturn position in ICRF */
    if (dso::planet_pos(dso::Planet::SATURN, t, rtb)) {
      fprintf(stderr, "ERROR Failed to compute Saturn position!\n");
      return 2;
    }
    /* compute third body acceleration for given epoch/position */
    a = dso::point_mass_acceleration(in.xyz, rtb, GM_Saturn, g);
    /* get COSTG result */
