...
int iers2010::gpt2(double dmjd, double *dlat, double *dlon, double *hell,
  ...
  for (int n = 0; n < maxl; n++) {
    fin >> dummy1 >> dummy2 >> pgrid[n][0] >> pgrid[n][1] >> pgrid[n][2] >>
        pgrid[n][3] >> pgrid[n][4] >> tgrid[n][0] >> tgrid[n][1] >>
        tgrid[n][2] >> tgrid[n][3] >> tgrid[n][4] >> qgrid[n][0] >>
        qgrid[n][1] >> qgrid[n][2] >> qgrid[n][3] >> qgrid[n][4] >>
        dtgrid[n][0] >> dtgrid[n][1] >> dtgrid[n][2] >> dtgrid[n][3] >>
        dtgrid[n][4] >> u[n] >> hs[n] >> ahgrid[n][0] >> ahgrid[n][1] >>
        ahgrid[n][2] >> ahgrid[n][3] >> ahgrid[n][4] >> awgrid[n][0] >>
        awgrid[n][1] >> awgrid[n][2] >> awgrid[n][3] >> awgrid[n][4];
    if (fin.fail()) {
      fin.close();
      return -2;
    }
  ...
  for (int k = 0; k < nstat; k++) {
    ...
    if (!ibilinear) {
      ...
      double t0 = tgrid[ix][0] + tgrid[ix][1] * cosfy + tgrid[ix][2] * sinfy +
                  tgrid[ix][3] * coshy + tgrid[ix][4] * sinhy;
      double p0 = pgrid[ix][0] + pgrid[ix][1] * cosfy + pgrid[ix][2] * sinfy +
                  pgrid[ix][3] * coshy + pgrid[ix][4] * sinhy;
      // specify humidity
      double q = qgrid[ix][0] + qgrid[ix][1] * cosfy + qgrid[ix][2] * sinfy +
                 qgrid[ix][3] * coshy + qgrid[ix][4] * sinhy;
      q /= 1000.0e0;
      // lapse rate of the temperature
      dt[k] = dtgrid[ix][0] + dtgrid[ix][1] * cosfy + dtgrid[ix][2] * sinfy +
      ...
      indx[3] = ipod1 * 72 + ilon1; // diagonal
      for (int l = 0; l < 4; l++) {
        int indxl = indx[l];
        // transforming ellipsoidial height to orthometric height:
        // Hortho = -N + Hell
        ...
        double t0 = tgrid[indxl][0] + tgrid[indxl][1] * cosfy +
                    tgrid[indxl][2] * sinfy + tgrid[indxl][3] * coshy +
                    tgrid[indxl][4] * sinhy;
        double p0 = pgrid[indxl][0] + pgrid[indxl][1] * cosfy +
                    pgrid[indxl][2] * sinfy + pgrid[indxl][3] * coshy +
                    pgrid[indxl][4] * sinhy;
        // humidity
        ql[l] = qgrid[indxl][0] + qgrid[indxl][1] * cosfy +
                qgrid[indxl][2] * sinfy + qgrid[indxl][3] * coshy +
                qgrid[indxl][4] * sinhy;
        ql[l] /= 1000.0e0;
        ...
        dtl[l] = dtgrid[indxl][0] + dtgrid[indxl][1] * cosfy +
                 dtgrid[indxl][2] * sinfy + dtgrid[indxl][3] * coshy +
                 dtgrid[indxl][4] * sinhy;
        dtl[l] /= 1000.0e0;
        ...
        // hydrostatic coefficient ah
        ahl[l] = ahgrid[indxl][0] + ahgrid[indxl][1] * cosfy +
                 ahgrid[indxl][2] * sinfy + ahgrid[indxl][3] * coshy +
                 ahgrid[indxl][4] * sinhy;
        ahl[l] /= 1000.0e0;
        // wet coefficient aw
        awl[l] = awgrid[indxl][0] + awgrid[indxl][1] * cosfy +
                 awgrid[indxl][2] * sinfy + awgrid[indxl][3] * coshy +
                 awgrid[indxl][4] * sinhy;
        awl[l] /= 1000.0e0;
      }

      double dnpod1 = std::abs(diffpod); // distance nearer point
      double dnpod2 = 1e0 - dnpod1;      // distance to distant point
