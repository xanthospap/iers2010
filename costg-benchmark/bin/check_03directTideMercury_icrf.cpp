...
constexpr const double GM_MERCURY  = 1.32712442076e20 / 6023600.0;
...
int main(int argc, char *argv[]) {
  ...
  if (formatD3Plot) {
    ...
    printf("#title Direct Tide Mercury (ephermeris: %s)\n", basename(argv[3]));
  }
  ...
  for (const auto &in : orbvec) {
    ...
    /* get Mercury position in ICRF */
    if (dso::planet_pos(dso::Planet::MERCURY, t, rtb)) {
      fprintf(stderr, "ERROR Failed to compute Mercury position!\n");
      return 2;
    }
    /* compute third body acceleration for given epoch/position */
    a = dso::point_mass_acceleration(in.xyz, rtb, GM_MERCURY, g);
    /* get COSTG result */
