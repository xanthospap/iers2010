...
iers2010::BlqIn::BlqIn(const char* filename)
  : __filename(filename),
    ...
    __eoheader(0)
{
  if (read_header()) throw std::runtime_error("[ERROR] BlqIn::BlqIn() failed");
}
...
///         *  0 : All ok
///         * 10 : Stream is closed
///         *  3 : Read MAX_HEADER_LINES before encountering the actual end of header
int
iers2010::BlqIn::read_header() noexcept
{
...
/// ----------------------------------------------------------------------------------
/// The function will try to peak the next's stations record (if any) and get the 
/// station's name. In detail, the function will:
...
/// - get the length of the remaining string; if it is 4 then it is interpreted as a
...
///          *  1  : error
int
iers2010::BlqIn::peak_next_station(std::string& sta) noexcept
{
  char line[MAX_HEADER_CHARS];
  const char* str;
...
iers2010::BlqIn::skip_next_station()
{
  ...
  if (!__istream.good()) return __istream.eof()?-1:1;
