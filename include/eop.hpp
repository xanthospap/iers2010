...
inline secularPole secular_pole(const MjdEpoch &tt) noexcept {
  ...
  const auto dt =
      tt.diff<DateTimeDifferenceType::FractionalYears>(j2000).years();
  return secularPole{55e0 + 1.677e0 * dt, 320.5e0 + 3.460e0 * dt};
}

/** @brief Compute ocean tide effect on polar motion (xp, yp), UT1 and LOD.
 *
 ...
 * See also the interp.f routine by IERS
 * (https://hpiers.obspm.fr/iers/models/interp.f) and the subroutines
 * PMUT1_OCEANS therein.
 *
...
/** @brief Compute periodic variations in UT1, LOD and Earth's rotation rate
 * (ω), due to tidal deformation (zonal tides) of the polar moment of inertia.
 *
...
class EopSeries {
  ...
  /** @brief Compute and add or remove zonal tidal variations (for UT1 and LOD).
   *
...
public:
  ...
  const auto &give_me_the_vector() const noexcept { return mvec; }
