...
void iers2010::dhtide::st1isem(const double *xsta, const double *xsun,
  ...
  constexpr double dhi{-0.0022e0}, dli{-0.0007e0};
  ...
  const double drsun{-3e0 / 4e0 * dhi * cosphi * cosphi * fac2sun *
                     (xs0m1 * sintwola - 2e0 * xsun[0] * xsun[1] * costwola) /
                     rsun2};

  const double drmon{-3e0 / 4e0 * dhi * cosphi * cosphi * fac2mon *
                     (xm0m1 * sintwola - 2e0 * xmon[0] * xmon[1] * costwola) /
                     rmon2};

  const double dnsun{3e0 / 2e0 * dli * sinphi * cosphi * fac2sun *
                     (xs0m1 * sintwola - 2e0 * xsun[0] * xsun[1] * costwola) /
                     rsun2};

  const double dnmon{3e0 / 2e0 * dli * sinphi * cosphi * fac2mon *
                     (xm0m1 * sintwola - 2e0 * xmon[0] * xmon[1] * costwola) /
                     rmon2};

  const double desun{-3e0 / 2e0 * dli * cosphi * fac2sun *
                     (xs0m1 * costwola + 2e0 * xsun[0] * xsun[1] * sintwola) /
                     rsun2};

  const double demon{-3e0 / 2e0 * dli * cosphi * fac2mon *
                     (xm0m1 * costwola + 2e0 * xmon[0] * xmon[1] * sintwola) /
                     rmon2};
