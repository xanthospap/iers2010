...
///           with the gradient model by Chen and Herring (1997). See Reference
///           1 and last lines of this subroutine. This function is a
///           translation/wrapper for the fortran APG subroutine, found here :
...
///
int iers2010::apg(double dlat, double dlon, double az, double el, double &d,
  ...
  for (int n = 1; n < nmax; n++) {
    N = n + 1;
    v[n + 1][0] = ((2 * N - 1) * z * v[n][0] - (N - 1) * v[n - 1][0]) /
  ...
  for (int m = 0; m < mmax; m++) {
    M = m + 1;
    v[m + 1][m + 1] =
        static_cast<double>(2 * M - 1) * (x * v[m][m] - y * w[m][m]);
    w[m + 1][m + 1] =
        static_cast<double>(2 * M - 1) * (x * w[m][m] + y * v[m][m]);
    if (m < mmax - 1) {
  ...
  d = 1e0 / (sin(el) * tan(el) + 0.0031e0) *
      (grn * cos(az) + gre * sin(az)); // mm
  d = d / 1000e0;                      // m
