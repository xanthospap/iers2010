...
void step2diu (const double* xsta,const double& fhr,const double& t,double* xcorsta)
{
  ...
  for (int i=0;i<3;i++) *(xcorsta+i) = .0e0;
  ...
  for (int j=0;j<31;j++) {
    ...
    double dr     = datdi[j][5]*2e0*sinphi*cosphi*sin(thetaf+zla)+
                    datdi[j][6]*2e0*sinphi*cosphi*cos(thetaf+zla);

    double dn     = datdi[j][7]*(cosphi*cosphi-sinphi*sinphi)*sin(thetaf+zla)+
                    datdi[j][8]*(cosphi*cosphi-sinphi*sinphi)*cos(thetaf+zla);

    // DE=DATDI(8,J)*SINPHI*COS(THETAF+ZLA)+
    // Modified 20 June 2007
    double de     = datdi[j][7]*sinphi*cos(thetaf+zla)-
  ...
  for (int i=0;i<3;i++) *(xcorsta+i) /= 1000e0;
